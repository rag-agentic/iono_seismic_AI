...
FILENAME='../json/ionosphere/stations_PBO.json'
...
df['network'] = df['properties.stntype'].apply(lambda x: x[0])
print(df.head())

header="""Plate Boundary Observatory (PBO) ground stations continuously collect high-precision GPS data, which is used for:
...
for iname_id,long,lat in zip(df['name_id'],df['longitude'],df['latitude']):
    print(iname_id,long,lat)
    #text.append(f"The earth station named {iname_id} is at longitude: {long:.2f}° and atitude: {lat:.2f}°")
...
with open(os.path.join(FOLDER_OUTPUT,f"ground_station_PBO.txt"), "wb") as file:
    file.write(header.encode())
    file.writelines(element.encode() + b'\n' for element in text)
    print("File written to disk")

for name in  df['name_id']:  # Afficher seulement les 5 premières stations
    print(f"\"{name}\",",end="")
